...
#
# Sensitivity analysis of baseline striatal dopamine transporter
# (DAT) moderation of the association between baseline
# neutrophil-to-lymphocyte ratio (NLR) and longitudinal motor
# progression in Parkinson's disease, restricted to the DAT
# imaging interval.
#
# Outcome:
...
#
# Output:
#   <PPMI_OUTPUT_ROOT>/09_NLR_MODERATION_STRIATAL_DAT_4Y
#
...
out_dir <- file.path(output_root, "09_NLR_MODERATION_STRIATAL_DAT_4Y")
...
# ---------------------------------------------------------
# Prepare raw PD-only dataset restricted to DAT imaging interval
# BL, V04, V06, V10 = years 0, 1, 2, 4
# ---------------------------------------------------------
...
write.csv(
    dat_long0,
    file.path(out_dir, "Shared_longitudinal_UPDRS_dataset_4Y_before_moderator_filter.csv"),
    row.names = FALSE
)
...
write.csv(
    ...
    file.path(out_dir, "Baseline_UPDRS_audit_4Y.csv"),
    row.names = FALSE
)

write.csv(
    baseline_updrs_join_audit,
    file.path(out_dir, "Baseline_UPDRS_join_audit_4Y.csv"),
    row.names = FALSE
)
...
time_points_4y <- c(0, 1, 2, 4)

# ---------------------------------------------------------
# Main 4Y moderation function
# ---------------------------------------------------------
run_striatal_moderation_4y <- function(
        data,
        dat_var,
        dat_label,
        file_stub
    ...
    cat("\n============================================================\n")
    cat("Running 4-year moderation model:", file_stub, "\n")
    cat("DAT moderator:", dat_var, "\n")
    cat("============================================================\n")
    
    # -----------------------------------------------------
    # Baseline DAT extraction and standardization
    # -----------------------------------------------------
    bl_dat_raw <- data %>%
        filter(EVENT_ID == "BL") %>%
        select(
    ...
    write.csv(
        bl_dat,
        file.path(out_dir, paste0(file_stub, "_baseline_DAT_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        baseline_dat_audit,
        file.path(out_dir, paste0(file_stub, "_baseline_DAT_audit_4Y.csv")),
        row.names = FALSE
    )
    ...
    write.csv(
        ...
        file.path(out_dir, paste0(file_stub, "_baseline_DAT_join_audit_4Y.csv")),
        row.names = FALSE
    )
    ...
    write.csv(
        ...
        file.path(out_dir, paste0(file_stub, "_Missingness_by_model_variable_including_outcome_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        missingness_by_predictor_covariate,
        file.path(out_dir, paste0(file_stub, "_Missingness_by_predictor_covariate_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        missingness_by_visit,
        file.path(out_dir, paste0(file_stub, "_Missingness_by_visit_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        participant_level_missingness,
        file.path(out_dir, paste0(file_stub, "_Participant_level_missingness_long_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        participant_missingness_summary,
        file.path(out_dir, paste0(file_stub, "_Participant_level_missingness_summary_4Y.csv")),
        row.names = FALSE
    )
    ...
    write.csv(
        dat,
        file.path(out_dir, paste0(file_stub, "_moderation_dataset_4Y_complete_case.csv")),
        row.names = FALSE
    )
    ...
    write.csv(
        desc,
        file.path(out_dir, paste0(file_stub, "_descriptives_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        visit_counts,
        file.path(out_dir, paste0(file_stub, "_visit_counts_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        site_counts,
        file.path(out_dir, paste0(file_stub, "_site_counts_4Y.csv")),
        row.names = FALSE
    )
    
    # -----------------------------------------------------
    # Main 4-year sensitivity model
    # -----------------------------------------------------
    lmm <- safe_lmer(
        UPDRS ~ baseline_NLR_z * DAT_BL_z * year_c +
            age + sex + bmi + duration_yrs + LEDD + DOMSIDE +
            (1 | SITE) + (1 | PATNO),
        data = dat,
        model_name = paste0(file_stub, " main 4Y NLR x DAT x time moderation model")
    )
    ...
    model_fit <- tibble(
        moderator = dat_var,
        dat_label = dat_label,
        model = "main_4Y_NLR_DAT_time_moderation",
        AIC = AIC(lmm),
        BIC = BIC(lmm),
    ...
    write.csv(
        fixef_tab,
        file.path(out_dir, paste0(file_stub, "_LMM_fixed_effects_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        anova_tab,
        file.path(out_dir, paste0(file_stub, "_LMM_TypeIII_ANOVA_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        randef_var,
        file.path(out_dir, paste0(file_stub, "_LMM_random_effects_variance_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        model_fit,
        file.path(out_dir, paste0(file_stub, "_LMM_model_fit_indices_4Y.csv")),
        row.names = FALSE
    )
    ...
    write.csv(
        ...
        file.path(out_dir, paste0(file_stub, "_simple_slopes_NLR_by_DAT_and_time_4Y.csv")),
        row.names = FALSE
    )
    ...
    write.csv(
        emm_df,
        file.path(out_dir, paste0(file_stub, "_predicted_UPDRS_trajectories_1p5SD_4Y.csv")),
        row.names = FALSE
    )
    ...
    p1 <- ggplot(
        ...
        scale_x_continuous(
            breaks = time_points_4y,
            labels = c("BL", "1", "2", "4")
        ) +
        scale_color_discrete(drop = TRUE, na.translate = FALSE) +
        scale_fill_discrete(drop = TRUE, na.translate = FALSE) +
        labs(
            title = paste0(dat_label, " moderates the association between baseline NLR and 4-year motor progression"),
            subtitle = "Sensitivity analysis restricted to the DAT imaging interval",
    ...
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Figure1_moderation_1p5SD_4Y.png")),
        plot = p1,
        width = 11,
        height = 5.8,
        dpi = 600
    ...
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Figure1_moderation_1p5SD_4Y.pdf")),
        plot = p1,
        width = 11,
        height = 5.8
    )
    ...
    write.csv(
        raw_df,
        file.path(out_dir, paste0(file_stub, "_raw_UPDRS_by_NLR_and_DAT_tertiles_4Y.csv")),
        row.names = FALSE
    )
    ...
    p2 <- ggplot(
        ...
        scale_color_discrete(drop = TRUE, na.translate = FALSE) +
        labs(
            title = paste0("Observed 4-year motor trajectories by baseline NLR and ", dat_label, " tertiles"),
            subtitle = "Sensitivity analysis: raw descriptive means",
            x = "Years from baseline",
    ...
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Figure2_moderation_raw_tertiles_4Y.png")),
        plot = p2,
        width = 11,
        height = 5.8,
        dpi = 600
    ...
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Figure2_moderation_raw_tertiles_4Y.pdf")),
        plot = p2,
        width = 11,
        height = 5.8
    )
    ...
    # baseline UPDRS-adjusted follow-up-only model
    # -----------------------------------------------------
    model_vars_sens <- c(
        "UPDRS",
        "baseline_UPDRS",
    ...
    lmm_sens <- safe_lmer(
        UPDRS ~ baseline_NLR_z * DAT_BL_z * year_c +
            ...
            (1 | SITE) + (1 | PATNO),
        data = dat_sens,
        model_name = paste0(file_stub, " 4Y baseline UPDRS-adjusted follow-up-only model")
    )
    ...
    model_fit_sens <- tibble(
        moderator = dat_var,
        dat_label = dat_label,
        model = "sensitivity_4Y_baseline_UPDRS_adjusted_followup_only",
        AIC = AIC(lmm_sens),
        BIC = BIC(lmm_sens),
    ...
    write.csv(
        dat_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_dataset_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        missingness_by_model_variable_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_missingness_by_model_variable_including_outcome_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        missingness_by_predictor_covariate_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_missingness_by_predictor_covariate_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        fixef_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_LMM_fixed_effects_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        anova_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_LMM_TypeIII_ANOVA_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        randef_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_random_effects_4Y.csv")),
        row.names = FALSE
    )
    
    write.csv(
        model_fit_sens,
        file.path(out_dir, paste0(file_stub, "_Sensitivity_baseline_UPDRS_adjusted_model_fit_indices_4Y.csv")),
        row.names = FALSE
    )
    
    # -----------------------------------------------------
    # Diagnostics for main 4Y model
    # -----------------------------------------------------
    diagnostic_df <- dat %>%
        mutate(
    ...
    write.csv(
        diagnostic_df,
        file.path(out_dir, paste0(file_stub, "_Diagnostic_values_4Y.csv")),
        row.names = FALSE
    )
    ...
    p_resid_fitted <- ggplot(
        ...
        labs(
            title = paste0(file_stub, ": residuals vs fitted values, 4Y sensitivity"),
            x = "Fitted values",
            y = "Residuals"
        )
    
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Diagnostic_residuals_vs_fitted_4Y.png")),
        plot = p_resid_fitted,
        width = 7,
        height = 5,
        dpi = 600
    ...
    p_qq_resid <- ggplot(
        ...
        theme_classic(base_size = 13) +
        labs(
            title = paste0(file_stub, ": Q-Q plot of residuals, 4Y sensitivity"),
            x = "Theoretical quantiles",
            y = "Sample quantiles"
        )
    
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Diagnostic_QQ_residuals_4Y.png")),
        plot = p_qq_resid,
        width = 7,
        height = 5,
        dpi = 600
    ...
    p_obs_fit <- ggplot(
        ...
        theme_classic(base_size = 13) +
        labs(
            title = paste0(file_stub, ": observed vs fitted values, 4Y sensitivity"),
            x = "Fitted values",
            y = "Observed ON-medication UPDRS-III"
        )
    
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Diagnostic_observed_vs_fitted_4Y.png")),
        plot = p_obs_fit,
        width = 7,
        height = 5,
        dpi = 600
    ...
    p_qq_ranef_patno <- ggplot(
        ...
        theme_classic(base_size = 13) +
        labs(
            title = paste0(file_stub, ": Q-Q plot of participant random intercepts, 4Y sensitivity"),
            x = "Theoretical quantiles",
            y = "Participant random intercepts"
        )
    
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Diagnostic_QQ_random_intercepts_PATNO_4Y.png")),
    ...
    p_qq_ranef_site <- ggplot(
        ...
        theme_classic(base_size = 13) +
        labs(
            title = paste0(file_stub, ": Q-Q plot of site random intercepts, 4Y sensitivity"),
            x = "Theoretical quantiles",
            y = "Site random intercepts"
        )
    
    ggsave(
        filename = file.path(out_dir, paste0(file_stub, "_Diagnostic_QQ_random_intercepts_SITE_4Y.png")),
    ...
    safe_check_model_png(
        model = lmm,
        filename = paste0(file_stub, "_Performance_check_model_4Y.png")
    )
    
    # -----------------------------------------------------
    # Summary text
    # -----------------------------------------------------
    sink(file.path(out_dir, paste0(file_stub, "_moderation_summary_4Y.txt")))
    
    cat("============================================================\n")
    cat("STRIATAL DAT moderation analysis - 4Y sensitivity analysis\n")
    cat("Restricted to DAT imaging interval, first 4 years\n")
    cat("============================================================\n\n")
    ...
    cat("Restriction:\n")
    cat("Analysis restricted to BL, V04, V06, and V10, corresponding to years 0, 1, 2, and 4.\n\n")
    ...
    cat("Optional sensitivity model: baseline UPDRS-adjusted follow-up-only model\n")
    cat("Rows =", nrow(dat_sens), "\n")
    cat("Unique PD subjects =", n_distinct(dat_sens$PATNO), "\n")
    cat("Unique sites =", n_distinct(dat_sens$SITE), "\n")
    cat("Unique participant-visits =", n_distinct(paste(dat_sens$PATNO, dat_sens$EVENT_ID)), "\n\n")
    ...
    cat("Fixed effects, baseline UPDRS-adjusted model:\n")
    print(fixef_sens, row.names = FALSE)
    cat("\n\n")
    
    cat("Type III ANOVA, baseline UPDRS-adjusted model:\n")
    print(anova_sens, row.names = FALSE)
    cat("\n\n")
    
    cat("Figures saved:\n")
    cat(paste0("- ", file_stub, "_Figure1_moderation_1p5SD_4Y.png/pdf\n"))
    cat(paste0("- ", file_stub, "_Figure2_moderation_raw_tertiles_4Y.png/pdf\n\n"))
    
    cat("Model assumption checks saved as diagnostic plots:\n")
    cat(paste0("- ", file_stub, "_Diagnostic_residuals_vs_fitted_4Y.png\n"))
    cat(paste0("- ", file_stub, "_Diagnostic_QQ_residuals_4Y.png\n"))
    cat(paste0("- ", file_stub, "_Diagnostic_observed_vs_fitted_4Y.png\n"))
    cat(paste0("- ", file_stub, "_Diagnostic_QQ_random_intercepts_PATNO_4Y.png\n"))
    cat(paste0("- ", file_stub, "_Diagnostic_QQ_random_intercepts_SITE_4Y.png\n"))
    cat(paste0("- ", file_stub, "_Performance_check_model_4Y.png\n"))
...
# ---------------------------------------------------------
res_caudate_l <- run_striatal_moderation_4y(
    data = dat_long0,
...
res_caudate_r <- run_striatal_moderation_4y(
    data = dat_long0,
...
res_putamen_l <- run_striatal_moderation_4y(
    data = dat_long0,
...
res_putamen_r <- run_striatal_moderation_4y(
    data = dat_long0,
...
write.csv(
    combined_fixef,
    file.path(out_dir, "COMBINED_moderation_4Y_LMM_fixed_effects_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_anova,
    file.path(out_dir, "COMBINED_moderation_4Y_LMM_TypeIII_ANOVA_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_model_fit,
    file.path(out_dir, "COMBINED_moderation_4Y_LMM_model_fit_indices_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_slopes,
    file.path(out_dir, "COMBINED_simple_slopes_NLR_by_DAT_and_time_4Y_all_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_predictions,
    file.path(out_dir, "COMBINED_predicted_UPDRS_trajectories_1p5SD_4Y_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_sensitivity_fixef,
    file.path(out_dir, "COMBINED_sensitivity_4Y_baseline_UPDRS_adjusted_fixed_effects_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_sensitivity_anova,
    file.path(out_dir, "COMBINED_sensitivity_4Y_baseline_UPDRS_adjusted_TypeIII_ANOVA_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_sensitivity_model_fit,
    file.path(out_dir, "COMBINED_sensitivity_4Y_baseline_UPDRS_adjusted_model_fit_indices_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_missingness_predictor,
    file.path(out_dir, "COMBINED_missingness_by_predictor_covariate_4Y_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_missingness_model,
    file.path(out_dir, "COMBINED_missingness_by_model_variable_including_outcome_4Y_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_participant_missingness,
    file.path(out_dir, "COMBINED_participant_level_missingness_summary_4Y_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_baseline_dat_audit,
    file.path(out_dir, "COMBINED_baseline_DAT_audit_4Y_all_DAT_moderators.csv"),
    row.names = FALSE
)

write.csv(
    combined_baseline_dat_join_audit,
    file.path(out_dir, "COMBINED_baseline_DAT_join_audit_4Y_all_DAT_moderators.csv"),
    row.names = FALSE
)
...
openxlsx::saveWorkbook(
    wb,
    file = file.path(out_dir, "NLR_striatal_DAT_moderation_4Y_sensitivity_outputs.xlsx"),
    overwrite = TRUE
)
...
# ---------------------------------------------------------
sink(file.path(out_dir, "COMBINED_NLR_striatal_DAT_moderation_4Y_sensitivity_summary.txt"))

cat("============================================================\n")
cat("4Y sensitivity analysis: baseline striatal DAT moderation\n")
cat("of baseline NLR-related motor progression\n")
cat("============================================================\n\n")
...
cat("Combined fixed effects, main 4Y models:\n")
print(combined_fixef, row.names = FALSE)
cat("\n\n")

cat("Combined Type III ANOVA, main 4Y models:\n")
print(combined_anova, row.names = FALSE)
cat("\n\n")

cat("Combined model fit indices, main 4Y models:\n")
...
cat("Excel workbook:\n")
cat(file.path(out_dir, "NLR_striatal_DAT_moderation_4Y_sensitivity_outputs.xlsx"), "\n\n")
...
# ---------------------------------------------------------
cat("\n============================================================\n")
cat("NLR x baseline DAT moderation 4Y sensitivity analyses completed.\n")
cat("Restricted to DAT imaging interval: BL, V04, V06, V10.\n")
cat("All outputs were saved to:\n")
cat(out_dir, "\n\n")
cat("Main combined summary:\n")
cat(file.path(out_dir, "COMBINED_NLR_striatal_DAT_moderation_4Y_sensitivity_summary.txt"), "\n\n")
cat("Excel workbook:\n")
cat(file.path(out_dir, "NLR_striatal_DAT_moderation_4Y_sensitivity_outputs.xlsx"), "\n")
cat("============================================================\n")