...
# Baseline striatal DAT moderation of baseline NLR effect on
# motor progression, restricted to the DAT imaging interval
#
# Outcome:
...
#   outputs/09_baseline_dat_moderation_4year_sensitivity
#
# Optional command-line usage:
#   Rscript R/09_baseline_dat_moderation_4year_sensitivity.R \
...
# ---------------------------------------------------------
# Prepare PD-only dataset restricted to DAT imaging interval
...
# ---------------------------------------------------------
# Baseline UPDRS for optional baseline-outcome adjusted sensitivity
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Main moderation function, restricted to first 4 years
# ---------------------------------------------------------
run_striatal_moderation_4y <- function(dat_var, dat_label, file_stub = dat_var) {
  ...
  write.csv(
    ...
    file.path(
      ...
      paste0(
        file_stub,
        "_baseline_DAT_standardization_parameters_4Y.csv"
      )
    ),
    row.names = FALSE
  )
  ...
  write.csv(
    ...
    file.path(out_dir, paste0(file_stub, "_Missingness_by_model_variable_4Y.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    ...
    file.path(out_dir, paste0(file_stub, "_Participant_level_missingness_4Y.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    ...
    file.path(
      out_dir,
      paste0(file_stub, "_LMM_convergence_and_singularity_4Y.csv")
    ),
    row.names = FALSE
  )
  ...
  # -----------------------------------------------------
  # Simple slopes of baseline NLR at each time point,
  ...
  # -----------------------------------------------------
  # Optional baseline UPDRS-adjusted follow-up-only model
  # This is included for consistency with reviewer-requested
  # baseline outcome adjustment, but the main 4Y sensitivity
  # model remains the BL-to-year-4 longitudinal model.
  # -----------------------------------------------------
  model_vars_sens <- c(
    "UPDRS",
    "baseline_UPDRS",
  ...
  png(
    filename = file.path(out_dir, paste0(file_stub, "_Performance_check_model_4Y.png")),
    width = 2400,
    height = 1800,
    res = 300
  )
  ...
  cat("============================================================\n")
  cat("STRIATAL DAT moderation analysis - sensitivity analysis\n")
...
cat("\n============================================================\n")
cat("NLR x baseline DAT moderation sensitivity analyses completed.\n")
