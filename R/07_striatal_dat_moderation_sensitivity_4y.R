...
# Striatal DAT Moderation Models Restricted to First 4 Years
#
# This script:
#   1. Restricts the dataset to Parkinson's disease participants
#   2. Restricts the longitudinal analysis to BL, V04, V06, and V10
...
#   4. Extracts baseline striatal DAT binding measures
#   5. Tests whether baseline striatal DAT binding moderates the
#      association between baseline PIS and 4-year motor progression
#   6. Runs separate moderation models for:
...
#      raw descriptive plots, and combined sensitivity summary tables
#
# Required input:
...
#
# Restriction:
#   BL, V04, V06, and V10 only
#
# Outputs:
#   outputs/striatal_dat_moderation_sensitivity_4y/
# =========================================================
...
out_dir   <- file.path("outputs", "striatal_dat_moderation_sensitivity_4y")
...
#
# Sensitivity analysis is restricted to the first 4 years:
#   BL  = 0
#   V04 = 1
#   V06 = 2
#   V10 = 4
...
sensitivity_events <- c("BL", "V04", "V06", "V10")

event_map_sensitivity <- event_map_all %>%
  dplyr::filter(EVENT_ID %in% sensitivity_events)

time_points <- event_map_sensitivity$year
...
# ---------------------------------------------------------
# Prepare PD-only dataset restricted to first 4 years
#
# PRIMDIAG:
...
cat("Number of PD rows after 4-year visit restriction:", nrow(df2), "\n")
...
if (dplyr::n_distinct(dat_long$PATNO) < 3) {
  stop("Too few unique PD participants are available for sensitivity moderation modeling.")
}

write.csv(
  dat_long,
  file.path(out_dir, "Shared_longitudinal_UPDRS_dataset_4Y.csv"),
  row.names = FALSE
)
...
# ---------------------------------------------------------
# Main 4-year moderation function
# ---------------------------------------------------------

run_striatal_moderation_4y <- function(dat_var, dat_label, file_stub = dat_var) {
  
  cat("\n------------------------------------------------------------\n")
  cat("Running 4-year sensitivity moderation model for:", dat_label, "\n")
  ...
  # -------------------------------------------------------
  # Merge baseline DAT into 4-year longitudinal UPDRS dataset
  # -------------------------------------------------------
  ...
  if (dplyr::n_distinct(dat$PATNO) < 3) {
    stop(
      paste0(
        "Too few unique participants for 4-year sensitivity model: ",
        dat_label
      )
    )
  }
  ...
  write.csv(
    dat,
    file.path(out_dir, paste0(file_stub, "_moderation_dataset_4Y.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    ...
    file.path(out_dir, paste0(file_stub, "_simple_slopes_PIS_by_DAT_and_time_4Y.csv")),
    row.names = FALSE
  )
  ...
  p1 <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = paste0(
        dat_label,
        " moderates the association between baseline PIS and 4-year motor progression"
      ),
      subtitle = "Sensitivity analysis restricted to BL, V04, V06, and V10",
      x = "Years",
  ...
  write.csv(
    raw_df,
    file.path(out_dir, paste0(file_stub, "_raw_descriptive_UPDRS_by_PIS_DAT_tertiles_4Y.csv")),
    row.names = FALSE
  )
  ...
  p2 <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = paste0(
        "Observed 4-year motor trajectories by baseline PIS and ",
        dat_label,
        " tertiles"
      ),
  ...
  ggplot2::ggsave(
    filename = file.path(out_dir, paste0(file_stub, "_Figure2_moderation_raw_4Y.png")),
    plot = p2,
    width = 11,
    height = 5.8,
    dpi = 600
  ...
  ggplot2::ggsave(
    filename = file.path(out_dir, paste0(file_stub, "_Figure2_moderation_raw_4Y.pdf")),
    plot = p2,
    width = 11,
    height = 5.8
  )
  ...
  cat("============================================================\n")
  cat("Striatal DAT moderation analysis: sensitivity analysis restricted to first 4 years\n")
  cat("============================================================\n\n")
...
# ---------------------------------------------------------
# Run all 4-year striatal DAT moderation models
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Combined overview tables across 4-year sensitivity models
# ---------------------------------------------------------
...
write.csv(
  combined_type3,
  file.path(out_dir, "Combined_moderation_TypeIII_ANOVA_4Y.csv"),
  row.names = FALSE
)
...
write.csv(
  combined_slopes,
  file.path(out_dir, "Combined_simple_slopes_PIS_by_DAT_and_time_4Y.csv"),
  row.names = FALSE
)
...
cat("\n============================================================\n")
cat("4-year striatal DAT moderation sensitivity analyses completed.\n")
